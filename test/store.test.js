...
describe("Stores", () => {
  ...
  describe("POST", () => {
    ...
    it("should not insert an entry for store if name is missing", (done)=>{
      chai.request(app).post('/stores').send({
          "name": "",
          "description": "An online store for all your needs",
        ...
        if (err) {
          ...
          res.body.should.have.property('errormessage').eql(`Store's name is required`);
          done();
      }
      })
    ...
    it("should not insert an entry for store if address is missing", (done)=>{
      chai.request(app).post('/stores').send({
        ...
        if (err) {
          ...
          res.should.have.status(400);
          res.body.should.have.property('errormessage').eql(`Store's address is required`);
          done();
      }
      })
    ...
    it("should not insert an entry for store if phone no is missing", (done)=>{
      chai.request(app).post('/stores').send({
        ...
        if (err) {
          ...
          res.should.have.status(400);
          res.body.should.have.property('errormessage').eql(`Store's phone no is required`);
          done();
      }
      })
  ...
  describe("PUT", ()=>{
    it("should update store's data", (done)=>{
      ...
      chai.request(app).put(`/stores/${id}`).send({
        "name": "Autin Kart",
        "description": "An online store for all your needs",
        "address": "Bailey road, India",
        "phone": "+1 2344567888"
  ...
  describe("DELETE", ()=>{
    it("should delete a store entry", (done)=>{
      const id = `64c8c01a5b0a637e1211710d`;
      chai.request(app).delete(`/stores/${id}`).send().end((err, res)=>{
        if (err) {
          console.log(err.message);
          done(err);
      } else {
